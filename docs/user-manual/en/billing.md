...
### Step 1: Exit survey
The cancellation window starts with a survey. The user is asked for:
...
5. A confirmation box the user must select.
...
5. Your survey answers are saved so NeuroTrader Journal can improve the service.
6. You may receive an invitation to return later.
...
## Billing emails
NeuroTrader Journal sends the main subscription lifecycle emails using the same visual style as the platform.
...
6. `Invitation to return` later if the subscription remains canceled.
