...
Por qué importa: determina los días de trading y el ritmo.  
Cómo llenarlo: elige una fecha que puedas cumplir sin forzar retornos irreales.

## 4) Días de trading
Qué es: cuántos días vas a operar entre hoy y la fecha objetivo.  
Cómo se calcula: automático usando días NYSE (feriados excluidos).  
Cuándo editarlo: si vas a operar menos o más días que lo que calcula el sistema.
...
## 8) % requerido en días de meta
Qué es: el % diario que necesitas en **días ganadores** para llegar a tu meta (después de considerar los días de pérdida).  
Por qué importa: traduce el plan a un objetivo diario realista.  
Cómo usarlo: úsalo como referencia de “día verde”, no como garantía diaria.
