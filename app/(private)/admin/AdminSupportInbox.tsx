...
function priorityLabel(priority: SupportTicketPriority | null | undefined, isEs: boolean) {
  if (priority === "urgent") return isEs ? "Urgente" : "Urgent";
  if (priority === "high") return isEs ? "Alta" : "High";
  if (priority === "low") return isEs ? "Baja" : "Low";
  return isEs ? "Normal" : "Normal";
}

function priorityClasses(priority: SupportTicketPriority | null | undefined) {
  if (priority === "urgent") return "border-rose-400/60 bg-rose-500/15 text-rose-100";
  if (priority === "high") return "border-amber-400/60 bg-amber-500/15 text-amber-100";
  if (priority === "low") return "border-slate-700 bg-slate-950/70 text-slate-400";
  return "border-sky-400/40 bg-sky-500/10 text-sky-100";
}

function statusClasses(status: SupportTicketStatus) {
  if (status === "waiting_support") return "border-amber-400/50 bg-amber-500/10 text-amber-100";
  if (status === "waiting_user") return "border-emerald-400/50 bg-emerald-500/10 text-emerald-100";
  if (status === "closed") return "border-slate-700 bg-slate-950/70 text-slate-400";
  return "border-sky-400/40 bg-sky-500/10 text-sky-100";
}
...
export default function AdminSupportInbox({ lang }: { lang: string }) {
  ...
  useEffect(() => {
    ...
    const channel = supabaseBrowser
      .channel(`admin-support-center-${userId}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "support_tickets" }, () => {
        refreshTickets();
      })
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "support_messages" }, () => {
        refreshTickets();
        if (selectedTicketId) refreshMessages(selectedTicketId);
      })
      .subscribe();
  ...
  }, [refreshMessages, refreshTickets, selectedTicketId, userId]);
  ...
  const filteredTickets = useMemo(() => {
    ...
    if (statusFilter !== "all") {
      next = next.filter((ticket) => ticket.status === statusFilter);
    }

    if (priorityFilter !== "all") {
      next = next.filter((ticket) => (ticket.priority ?? "normal") === priorityFilter);
    }
    ...
    return [...searched].sort((a, b) => {
      const ap = priorityWeight[String(a.priority ?? "normal")] ?? 2;
      const bp = priorityWeight[String(b.priority ?? "normal")] ?? 2;
      if (ap !== bp) return ap - bp;
      return new Date(b.last_message_at ?? b.created_at ?? 0).getTime() - new Date(a.last_message_at ?? a.created_at ?? 0).getTime();
    });
  }, [assigneeFilter, priorityFilter, query, statusFilter, tickets, userId]);
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-[11px] uppercase tracking-[0.24em] text-slate-500">
            {L("Admin Support Center", "Centro de soporte admin")}
          </p>
          <h2 className="mt-2 text-lg font-semibold text-slate-100">
            {L("Priority queue, staff replies, and service agent", "Cola por prioridad, respuestas del staff y agente de servicio")}
          </h2>
          <p className="mt-1 text-sm text-slate-400">
            {L(
              "Every user ticket lands here. Classify priority, assign ownership, reply as staff, or let the service agent answer clear questions first.",
              "Todos los tickets de usuarios llegan aquí. Clasifica prioridad, asigna dueño, responde como staff o deja que el agente responda preguntas claras primero."
            )}
          </p>
        </div>
        <div className="flex flex-wrap gap-3">
          ...
          >
            {L("Refresh support queue", "Actualizar cola")}
          </button>
          <button
            type="button"
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <div className="rounded-2xl border border-rose-500/25 bg-rose-500/10 p-4">
          <div className="text-[10px] uppercase tracking-[0.2em] text-rose-200">{L("Urgent priority", "Prioridad urgente")}</div>
          <div className="mt-2 text-2xl font-semibold text-rose-100">{queueStats.urgent}</div>
          <p className="mt-1 text-xs text-rose-100/75">
            {L("Review first. These are sorted to the top of the queue.", "Revisar primero. Estos se ordenan arriba de la cola.")}
          </p>
        </div>
        <div className="rounded-2xl border border-amber-500/25 bg-amber-500/10 p-4">
          <div className="text-[10px] uppercase tracking-[0.2em] text-amber-200">{L("High priority", "Prioridad alta")}</div>
          <div className="mt-2 text-2xl font-semibold text-amber-100">{queueStats.high}</div>
          <p className="mt-1 text-xs text-amber-100/75">
            {L("Needs faster staff attention after urgent tickets.", "Necesita atención rápida después de urgentes.")}
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[0.9fr,1.1fr]">
        <section className="rounded-3xl border border-slate-800 bg-slate-900/60 p-5">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            {(["all", "urgent", "high", "normal", "low"] as const).map((value) => (
              <button
                key={value}
                type="button"
              ...
              >
                {value === "all" ? L("All priorities", "Todas") : priorityLabel(value, isEs)}
              </button>
            ))}
          </div>
          ...
          <div className="mt-4 space-y-2">
            {loading ? (
              <div className="rounded-2xl border border-slate-800 bg-slate-950/50 p-4 text-sm text-slate-400">
                {L("Loading support queue…", "Cargando cola de soporte…")}
              </div>
            ) : null}
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex shrink-0 flex-col items-end gap-1">
                    <span className={`rounded-full border px-2 py-1 text-[10px] ${priorityClasses(ticket.priority)}`}>
                      {priorityLabel(ticket.priority, isEs)}
                    </span>
                    <span className={`rounded-full border px-2 py-1 text-[10px] ${statusClasses(ticket.status)}`}>
              ...
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2 text-[11px]">
                    <span className={`rounded-full border px-2 py-1 ${priorityClasses(selectedTicket.priority)}`}>
                      {L("Priority", "Prioridad")}: {priorityLabel(selectedTicket.priority, isEs)}
                    </span>
                    <span className={`rounded-full border px-2 py-1 ${statusClasses(selectedTicket.status)}`}>
                      {L("Status", "Estado")}: {ticketStatusLabel(selectedTicket.status, isEs)}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <select
                    aria-label={L("Ticket priority", "Prioridad del ticket")}
                    value={selectedTicket.priority ?? "normal"}
                    onChange={async (event) => {
                      await updateSupportTicket(selectedTicket.id, { priority: event.target.value as SupportTicketPriority });
                      await refreshTickets();
                      setNotice(L("Ticket priority updated.", "Prioridad del ticket actualizada."));
                    }}
