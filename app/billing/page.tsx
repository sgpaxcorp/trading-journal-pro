...
export default function BillingPage() {
  ...
  function handleContinue(plan: PlanId) {
    // En el futuro aquí puedes llamar a un API para guardar el plan
      ...
      <div className="w-full max-w-5xl rounded-3xl border border-slate-800 bg-slate-950/95 shadow-2xl p-6 md:p-10">
        ...
        <header className="mb-6 md:mb-8">
          ...
          <p className="text-xs md:text-sm text-slate-400 mt-2 max-w-2xl">
            ...
            continue, we&apos;ll take you to your setup page to configure your growth
            plan and dashboard.
          </p>
        </header>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              type="button"
              onClick={() => handleContinue(selectedPlan)}
              className="px-5 py-2.5 rounded-xl bg-emerald-400 text-slate-950 text-xs md:text-sm font-semibold hover:bg-emerald-300 transition"
            >
              Continue with {selectedPlan === "standard" ? "Standard" : "Professional"}
            </button>
            <Link
              href="/start"
