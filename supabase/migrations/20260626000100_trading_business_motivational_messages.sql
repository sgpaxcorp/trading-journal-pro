with annual_messages as (
  select
    locale,
    day_of_year,
    case
      when locale = 'es' then (array[
  ...
  from generate_series(1, 366) as day_of_year
  cross join (values ('en'), ('es')) as locales(locale)
)
...
select
  concat('motivation-', locale, '-day-', lpad(day_of_year::text, 3, '0')),
  locale,
  title,
  body,
  null,
